...
import defaultImage from '../../assets/avatar-default-icon.png'
import {
    Wrapper, CenteredBox,
    DoctorCard,
    MainContent,
...
const BookingSuccess = () => {
    ...
    const handleDownloadImage = async () => {
        ...
        const canvas = await html2canvas(contentRef.current, {
            useCORS: true,
            scale: 2, // chất lượng ảnh cao hơn
        });
        const image = canvas.toDataURL('image/png');

        const link = document.createElement('a');
        link.href = image;
        link.download = `phieu-kham-${appointment?.stt || 'khong-ro'}.png`;
        link.click();
    };
    return (
        ...
        <Wrapper>
            <div ref={contentRef}>
                ...
                <DoctorCard>
                    <Avatar
                        size={56}
                        icon={<UserOutlined />}
                        src={isHospital ? `${import.meta.env.VITE_APP_BACKEND_URL}${hospital?.thumbnail}` : `${import.meta.env.VITE_APP_BACKEND_URL}${doctor?.user?.avatar}` || defaultImage}
                    />
            ...
            <StickyFooter>
                ...
                <ButtonComponent
                    ...
                    size="large"
                    style={{ width: "50%", fontWeight: "bold", fontSize: 16 }}
                    onClick={handleDownloadImage} // Gắn sự kiện tại đây
                >
                    Lưu lại phiếu
                </ButtonComponent>
            </StickyFooter>
